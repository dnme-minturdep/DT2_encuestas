# **Guía para una encuesta** {#guiaencuesta}
...
## Definición de la población bajo estudio y las unidades de análisis

Ligada al planteamiento del propósito inicial de la investigación se encuentra la definición de cuál será la **población**[^guia_encuestas-1] **o universo bajo estudio**, entendiendo a ésta última como al conjunto de elementos que pretenden estudiarse. Definir la población bajo estudio implica tres pasos fundamentales:

[^guia_encuestas-1]: Si bien aquí se plantea indistintamente, algunos autores prefieren utilizar únicamente el término población, dado que consideran que el universo es "un conjunto infinito de datos", que son aplicados a la población (Hernández Sampieri, Fernández Collado, Baptista Lucio, 1998).
...
## Padrón de establecimientos de alojamiento turísticos
...
## Estudios por muestreo. Tipos de muestreo.
...
-   La muestra que de él resulta puede estar sesgada (es decir, puede haber sobre-representación de algún grupo y/ sub-representación de otro grupo).
...
Una forma adecuada de seleccionar una muestra con este procedimiento es enumerando a todos los elementos de la población (por ejemplo, a cada establecimiento del padrón de alojamientos habilitados) secuencialmente, desde 1 hasta N. Luego, se debe realizar una selección aleatoria de las unidades[^guia_encuestas-2].

[^guia_encuestas-2]: Programas informáticos de uso muy difundido como el Excel contemplan una función generadora de números aleatorios. No obstante, el "sorteo" puede realizarse de cualquier forma, incluso manualmente, mientras se respete el criterio de completa aleatoriedad.
...
Como puede deducirse, al establecer el margen de error con el que se trabajará se debe respetar la profundidad que la investigación requiere, o bien, reducir el alcance de los objetivos de la misma. Si bien siempre es preferible un margen de error más pequeño, lograrlo implica cada vez mayores costos (económicos, humanos, de tiempo), y por tanto es preciso analizar ventajas y desventajas para arribar a la definición a un nivel de error eficiente[^guia_encuestas-3].

[^guia_encuestas-3]: Cabe señalar que los estudios censales carecen, por definición, de errores estadísticos. No obstante, existen otro tipo de errores que afectan a los resultados obtenidos, tanto por muestra como por censo y que se denominan "errores no muestrales" o "errores no estadísticos" y, a diferencia de los errores estadísticos es imposible (o harto complejo y muy costoso, en el mejor de los casos) estimar la magnitud de los mismos. El hecho de que muchas veces se realicen estudios por muestra se relaciona tanto con cuestiones operativas (menores costos) como con la posibilidad de reducir los errores no muestrales, al realizar un relevamiento de campo de mejor calidad centrado en pocas unidades, como se mencionó más arriba.

    Vale la pena citar un ejemplo. En Argentina, el mercado de trabajo es estudiado fundamentalmente a través de la Encuesta Permanente de Hogares (EPH) (tasas de actividad, desocupación, informalidad, etc.), una encuesta relevada desde hace varias décadas a cargo de personal especializado en el conocimiento de la temática y del desarrollo del operativo de campo. En general, el margen de error es cercano o inferior al 1%, con lo que si se estima que la tasa de desocupación es 8%, el valor poblacional se ubica entre el 7% y el 9%. Ahora bien, los Censos de Población también incluyen preguntas sobre la situación laboral de las personas, y si bien toda la población es censada, el relevamiento se hace en un sólo día, sin las posibilidades de revisión y corrección en gabinete, y su ejecución queda a cargo de cientos de miles de personas que son entrenadas durante apenas unas pocas horas. En el año 2001, el censo de población se desarrolló en noviembre, a pocos días del estallido de finales de ese año, en un contexto más que convulsionado, que hizo que miles de personas salieran a censar sin siquiera haber recibido una capacitación básica. El resultado del censo arrojó una tasa de desocupación sustancialmente mayor a los valores que venía brindando la EPH. Esto ocurrió porque, al desconocer los conceptos básicos necesarios al indagar sobre aspectos laborales, muchos censistas calificaron como "desocupadas" a personas que, desde el punto de vista de las recomendaciones internacionales para la medición del empleo (en base a lo cual fue construida la cédula censal) debían ser calificadas como "ocupadas" pese no tener un "trabajo estable" (trabajadores informales, changarines, trabajo por horas en casas de familia). En este ejemplo queda en evidencia, y nada más ni nada menos que con un censo de población, cómo los errores no muestrales son más "peligrosos" que los errores estadísticos.
...
#### La representatividad y el azar
...
##### Procedimiento de selección de unidades dentro de los estratos
...
En virtud de lo expuesto, el **muestreo estratificado con asignación proporcional y proceso de  selección sistemática** se revela como la mejor opción para las encuestas de usos de alojamientos turísticos, allí donde la población alcanza un tamaño destacado.\
...
Cabe mencionar que, en este último caso, al momento de construir los ponderadores (véase apartado correspondiente), deberá contemplarse devolver a las unidades de dichos estratos el peso que originalmente tenían (es decir, aquel que corresponde a sus probabilidades de selección).
...
##### Estratos vs dominios de análisis
...
a)  reflejar en la muestra la estructura poblacional, en torno a alguna variable que se juzgue relevante para el objetivo de estudio;

b)  reducir la dispersión general de la población en torno a la variable de estudio que se considere más relevante para el estudio;

c)  lograr mayor precisión en los resultados;

d)  reducir el tamaño de muestra,
...
#### Ejemplo de estratificación y selección de muestras al interior de cada estrato
...
```{r padronY, echo=FALSE, fig.cap=" Padrón de establecimientos de la localidad Y" }
knitr::include_graphics('imagenes/Tabla 1.png')
```

##### Definición de los estratos en la localidad Y
...
```{r localidadY, echo=FALSE, fig.cap=" Estratos de establecimientos de la localidad Y" }
knitr::include_graphics('imagenes/Figura 1.png')
```
...
```{r criterios_estratos, echo=FALSE, fig.cap=" Criterios de estratificación y definición de los estratos" }
knitr::include_graphics('imagenes/Tabla 2.jpg')
```

##### Determinación del tamaño muestral de cada estrato

Como se mencionó en párrafos anteriores, para determinar el tamaño de muestra de cada estrato, es preciso que previamente se haya determinado el tamaño de muestra total para la localidad (es decir, considerando a todos los estratos). En este ejemplo no se desarrollará el proceso de cálculo del tamaño total, sino que se asumirá como dato existente. En este caso, se considerará un tamaño de muestra total de 15 casos[^guia_encuestas-4].

[^guia_encuestas-4]: Como puede apreciarse, el tamaño de muestra total es muy pequeño. En este ejemplo se define tal tamaño solo con fines ilustrativos, más aún considerando que el padrón solo consta de 30 establecimientos.
...
El equipo técnico de la provincia X tomó la decisión de que la asignación de casos por estrato se ajuste al peso que cada uno tiene sobre el universo total de establecimientos, obviamente restando los alojamientos de 4 y 5 estrellas y el establecimiento de 210 plazas, perteneciente este último al estrato "Hoteles 3 estrellas, Apart hotel y Boutique, más de 200 plazas". El peso de cada estrato se definirá en función de la cantidad de plazas disponibles del mismo, ya que ésta será una de las principales variables de análisis[^guia_encuestas-5]. En el siguiente cuadro se realiza el cálculo del peso de cada estrato sobre el total de plazas, luego de haber sustraído las plazas correspondientes a los establecimientos previamente mencionados.

[^guia_encuestas-5]: Otra variable en función de la cual se pueden calcular los pesos de los estratos podría ser habitaciones disponibles, siempre y cuando las habitaciones sean la (o una de las) unidad/es de análisis.

```{r cantidad_de_plazas, echo=FALSE, fig.cap="" }
knitr::include_graphics('imagenes/Tabla 2A.jpg')
```

```{r peso_de_estrato, echo=FALSE, fig.cap="" }
knitr::include_graphics('imagenes/Tabla 2B.jpg')
```
...
```{r casos_a_realizar, echo=FALSE, fig.cap="" }
knitr::include_graphics('imagenes/Tabla 2C.jpg')
```

Una vez que determinados los tamaños de muestra de cada estrato, se procede con la selección de los casos.

##### Selección de los casos al interior de cada estrato
...
```{r muestra_interior, echo=FALSE, fig.cap="Selección de las muestras al interior de cada estrato" }
knitr::include_graphics('imagenes/Tabla 3.jpg')
```
...
```{r muestra_final, echo=FALSE, fig.cap="Muestra final de establecimientos" }
knitr::include_graphics('imagenes/Tabla 4.jpg')
```
...
### Construcción del formulario de registro (cuestionario o planilla): definición de variables fundamentales
...
#### Variables e indicadores en una encuesta de alojamientos turísticos
...
-   habitaciones o unidades ocupadas[^guia_encuestas-7];
...
[^guia_encuestas-7]: En establecimientos no hoteleros como complejos de cabañas, apart hoteles, complejos de departamentos y campings, ya no se mide la ocupación y disponibilidad en habitaciones, sino en la misma unidad. En los complejos de cabañas, la unidad es la misma cabaña; en los apart hoteles y complejos de departamentos, el departamento; en los campings, la parcela.
...
```{r residencia_habitual, echo=FALSE, fig.cap="Lugar de residencia habitual" }
knitr::include_graphics('imagenes/Figura 2.jpg')
```

```{r residentes, echo=FALSE, fig.cap="Lugar de residencia habitual. Residentes de Argentina" }
knitr::include_graphics('imagenes/Figura 2.1.jpg')
```

\

```{r no_residentes, echo=FALSE, fig.cap="Lugar de residencia habitual. No residentes de Argentina" }
knitr::include_graphics('imagenes/Figura 2.2.jpg')
```

```{r america_latina, echo=FALSE, fig.cap="Lugar de residencia habitual. No residentes de Argentina. América" }
knitr::include_graphics('imagenes/Figura 2.2.1.jpg')
```

```{r europa, echo=FALSE, fig.cap="Lugar de residencia habitual. No residentes de Argentina. Europa" }
knitr::include_graphics('imagenes/Figura 2.2.2.jpg')
```
...
```{r ocupados, echo=FALSE, fig.cap="Tipos de ocupados" }
knitr::include_graphics('imagenes/Tabla 5.jpg')
```
...
Sin perder de vista las particularidades provinciales/departamentales/municipales que encierra la definición de cada clase de establecimiento, producto de las normativas vigentes en cada jurisdicción, en este apartado se planteará una propuesta mínima de clasificación de los mismos. La misma es empleada en los padrones del MINTUR y puede ser útil para establecer equivalencias entre las diferentes definiciones adoptadas en cada jurisdicción.

```{r tipos_alojamientos, echo=FALSE, fig.cap="Tipos de alojamientos turísticos" }
knitr::include_graphics('imagenes/Figura 3.jpg')
```

```{r establecimientos_hoteleros, echo=FALSE, fig.cap="Tipos de alojamientos turísticos. Establecimientos hoteleros" }
knitr::include_graphics('imagenes/Figura 3.1.jpg')
```

```{r establecimientos_para_hoteleros, echo=FALSE, fig.cap="Tipos de alojamientos turísticos. Establecimientos para-hoteleros" }
knitr::include_graphics('imagenes/Figura 3.2.jpg')
```

## Trabajo de campo

### Frecuencia de relevamiento y período de referencia de los datos relevados
...
```{r grupo_a_b, echo=FALSE, fig.cap="Tabla de Grupos" }
knitr::include_graphics('imagenes/Tabla 5A.jpg')
```
...
En la Tabla 6 se presentan las ventajas, limitaciones y requerimientos de cada forma de relevamiento. Cabe mencionar que, a pesar de sus pros y contras, todas son alternativas válidas a la hora de ejecutar el trabajo de campo mientras se respeten los criterios metodológicos básicos que deben guiar el trabajo de campo.

```{r relevamiento, echo=FALSE, fig.cap="Ventajas, limitaciones y requerimientos por forma de relevamiento" }
knitr::include_graphics('imagenes/Tabla 6.jpg')
```

### Otras consideraciones acerca del trabajo de campo

En relación al personal a cargo de la recolección de datos, como se desprende dela figura anterior, las encuestas pueden ser realizadas por personas que trabajen dentro de la institución y que estén afectadas a distintas tareas, o por encuestadores específicamente destinados a este trabajo. En la mayoría de las jurisdicciones, por razones de disponibilidad de recursos humanos, el trabajo de campo es ejecutado por el mismo personal de las entidades de turismo.
...
A continuación se presenta un modelo de planilla para la carga de los datos, que puede ser utilizada por los municipios y/o provincias, ya sea para producir datos propios o recopilar datos producidos por terceros. Para seguir en la misma línea, se utilizaron algunos de los establecimientos considerados en el ejemplo de estratificación y selección de muestras al interior de cada estrato (apartado 2.5.2.8). Asimismo, se asumió que el relevamiento se realiza los días lunes y miércoles, preguntando el lunes por las plazas ocupadas y plazas disponibles del sábado anterior, y el día miércoles por las plazas ocupadas del martes previo.

```{r tabla_6a, echo=FALSE, fig.cap="Modelo de planilla para la carga de los datos" }
knitr::include_graphics('imagenes/Tabla 6A.jpg')
```

### Consistencia y validación de la información

Una vez que han sido ingresados los casos a la base de datos, es necesario llevar adelante un proceso de **validación**[^guia_encuestas-8], lo cual implica realizar un análisis de consistencia interna de los datos y de situaciones extrañas o no esperadas en los valores de los datos. Durante la validación pueden detectarse **errores** y/o **advertencias**. Un ejemplo de error sería encontrar que la cantidad de plazas ocupadas supere a la cantidad de plazas disponibles[^guia_encuestas-9]. Por otro lado, una advertencia se podría presentar cuando en un establecimiento se produce una variación notoria en la cantidad de plazas ocupadas, en un período en que se esperaba que ese valor estuviese estancado. Si durante el proceso de validación se detecta algún dato inconsistente o alguna advertencia, lo adecuado es volver a contactar al encuestado para solicitarle el dato correcto en el caso de un error o para constatar si lo que surgió como advertencia es un error o una situación que tuvo lugar realmente.

[^guia_encuestas-8]: La validación no es exclusiva de esta etapa y es preciso que se definan pautas de consistencia para las diferentes etapas del campo (cuando se completa el formulario, cuando el mismo es recepcionado por un supervisor, etc.). No obstante, en esta etapa las herramientas informáticas permiten validar la información en forma certera, pudiendo incluir además información externa (por ejemplo, si algún valor se aleja sustancialmente de la media del estrato o de información suministrada anteriormente por el mismo establecimiento, casos en los que es preciso chequear que la información brindada sea la correcta).

    \

[^guia_encuestas-9]: Sin embargo, puede ocurrir que un establecimiento, en períodos de alta demanda, utilice sus plazas supletorias. En esos casos, se debe corregir el número correspondiente a las plazas disponibles.
...
### Ponderadores o factores de expansión
...
```{r peso, echo=FALSE, fig.cap="Peso de cada estrato en la población y muestra" }
knitr::include_graphics('imagenes/Tabla 7.jpg')
```
...
Para afianzar las ideas desarrolladas en este apartado, a continuación se describirá el proceso de construcción de factores de expansión para las muestras seleccionadas en el apartado 2.5.2.8.

Recuérdese que la muestra final seleccionada fue:

```{r tabla_7a, echo=FALSE, fig.cap="Muestra final" }
knitr::include_graphics('imagenes/Tabla 7 A.jpg')
```
...
```{r tabla_7, echo=FALSE, fig.cap="" }
knitr::include_graphics('imagenes/tabla 7B.jpg')
```
...
```{r indicadores, echo=FALSE, fig.cap="Indicadores, información necesaria para su construcción y procedimiento de estimación" }
knitr::include_graphics('imagenes/Tabla 8.jpg')
```
...
[^guia_encuestas-10]: Recuérdese que en algunas jurisdicciones la cantidad de plazas y/o habitaciones ofrecidas pueden sufrir variaciones (aumentos o bajas) a lo largo del año.
...
```{r 1_2_estrellas, echo=FALSE, fig.cap="IHoteles 1 y 2 estrellas, menos de 200 plazas" }
knitr::include_graphics('imagenes/Hoteles 1 y 2 estrellas, menos de 200 plazas.jpg')
```

```{r 3_estrellas, echo=FALSE, fig.cap="Hoteles 3 estrellas,  Apart hotel y Boutique, menos de 200 plazas" }
knitr::include_graphics('imagenes/Hoteles 3 estrellas,  Apart hotel y Boutique, menos de 200 plazas.jpg')
```

```{r 3_estrellas_mas_200, echo=FALSE, fig.cap="Hoteles 3 estrellas,  Apart hotel y Boutique, más de 200 plazas" }
knitr::include_graphics('imagenes/Hoteles 3 estrellas,  Apart hotel y Boutique, mas de 200 plazas.jpg')
```

```{r 4_5_estrellas_menos_200, echo=FALSE, fig.cap="Hoteles 4 y 5 estrellas, menos de 200 plazas" }
knitr::include_graphics('imagenes/Hoteles 4 y 5 estrellas, menos de 200 plazas.jpg')
```

```{r 4_5_estrellas_mas_200, echo=FALSE, fig.cap="Hoteles 4 y 5 estrellas, más de 200 plazas" }
knitr::include_graphics('imagenes/Hoteles 4 y 5 estrellas, mas de 200 plazas.jpg')
```

```{r para_hoteleros, echo=FALSE, fig.cap="Establecimientos parahoteleros" }
knitr::include_graphics('imagenes/Establecimientos parahoteleros.jpg')
```
...
```{r total_localodad_y, echo=FALSE, fig.cap="." }
knitr::include_graphics('imagenes/figuras-parte123-01.jpg')
```
